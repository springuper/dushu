...
```
**补全要求**：
- 覆盖文本中出现但未被事件引用到的零散人物/地点。
- 与事件引用的 `name/aliases` 做快速匹配，对齐后生成 canonical ID；无匹配时保留独立候选。

#### 2.4.5 对齐与消歧（人物/地点/关系）
1. **快速匹配**：规范化名称/别名，精确或交叉匹配。
2. **LLM 判断**：对可能重复的候选，传入已有记录与新记录，请求 `shouldMerge + confidence`。
3. **标记与合并**：
   - `shouldMerge=true && confidence>=0.7`：标记 duplicate 并生成 merge 建议。
   - 低置信度或拒绝合并：保守创建新记录，由人工审核。
4. **回填引用**：事件中的 `actors/location/relationships` 引用用 canonical ID 替换；若无 ID 则保留原名并在审核中提示“未匹配”。
